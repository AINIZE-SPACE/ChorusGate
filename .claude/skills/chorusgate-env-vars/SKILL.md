...
| `1d7f1c1` | `reply-engine.ts` / `claude.ts` / `claude-stream.ts` 动态读 `PERMISSION_MODE` | 删顶层 const，inline 读 |
...
```bash
src/providers/claude.ts:27 + src/providers/claude-stream.ts:44 (重复)   const CLAUDE_BIN = process.env.CLAUDE_BIN || "claude"

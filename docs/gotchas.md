...
### 7. Slack 消息身份错误 — 显示管理员用户名而非 Bot**现象**：ChorusGate/Claude 在 Slack 发消息时，消息显示为 Slack 管理员用户名 + "APP" 标注，而不是 Bot 自己的名称（如"小克"）。  
...
### 8. bot_message 的作者身份字段与人类不同（#144）
**现象**：解析 bot 消息时只读 `event.user`，丢失"这是 bot"的事实，无法跟踪 bot 间消息。
**原因**：bot 消息（`subtype: "bot_message"`）的主标识是 `bot_id`（B 前缀），`user`（U 前缀）仅在该 bot 有 user identity 时才出现；老式 bot 可能只有 `bot_id` + `username`，没有 `user`。人类消息则只有 `user`。
**修复**：所有解析路径统一走 `parseUser()`（`src/user-identity.ts`），按 `subtype === "bot_message"` / `bot_id` / `bot_profile` 判定 BOT，输出 `user_id` / `user_type` / `bot_id` / `is_bot` 富对象；`isBotMessage()` 提供快速判断。
**教训**：判断"消息是否来自 bot"不能只看 `user` 字段，bot 消息的 `user` 是可选的。

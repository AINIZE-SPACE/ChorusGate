...
Gateway 是**代理层**，不做人设。身份由 Provider 的人设文件决定：

| Provider | 人设文件 | Session 存储 |
|----------|---------|-------------|
| **CC** (小克) | 项目 `CLAUDE.md` + `~/.claude/CLAUDE.md` | `~/.claude/projects/<hash>/` |
| **CX** (小扣) | 项目 `AGENTS.md` + `.agents/` + `.codex/config.toml` | `~/.codex/sessions/` |
| **Gateway** | (无) | `memory/sessions.md` (路由 meta only) |

`CLAUDE.md` 归 CC，`AGENTS.md` 归 CX。Gateway 不读写这些文件来做身份决策，
只根据 `.env` 中的 `GATEWAY_PROVIDER` 决定 spawn 哪个 runtime。

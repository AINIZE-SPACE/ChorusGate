...
### Gateway 与 Provider 的身份分离

Gateway 是**代理层**，不应携带人设。身份（小克 / 小扣）由 Provider 宿主决定：

| 层 | 配置文件 | 职责 |
|------|------|------|
| Gateway | `CLAUDE.md` | 架构、命令、开发约定 |
| Claude Code | 项目 `CLAUDE.md` + `~/.claude/CLAUDE.md` | 小克的人设和行为指令 |
| Codex | 项目 `AGENTS.md` + `.agents/` + `.codex/config.toml` | 小扣的人设和行为指令 |

Gateway 启动时根据 `.env` 中的 `GATEWAY_PROVIDER` 和 `GATEWAY_COMMAND_PREFIX` 路由到不同的 runtime，
但 Gateway 本身的 `CLAUDE.md` 只写项目架构信息，不绑定某个 provider 的人设。

### 多 Gateway 实例并行

可以在不同目录启动多个 Gateway 实例（如 CC 网关 + Codex 网关并行运行）。
关键要求：
- 每个实例使用**不同的 Slack App**（不同的 token 对）
- 每个 Gateway 的 `.gateway/` 控制目录独立（分别在不同项目目录下）
- `memory/sessions.md` 按项目目录独立，互不干扰
- Claude Code memory 目录按项目路径 hash 独立
